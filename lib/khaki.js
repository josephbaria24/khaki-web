...
export function formatStamp(dateStr) {
  ...
  const time = formatClock(dateStr);
  const now = new Date();
...
export function isConversationPath(pathname) {
  return /\/task\/[^/]+\/chat\/?$/.test(pathname || "");
}

export function isFillPage(pathname) {
  return pathname === "/messages" || isConversationPath(pathname);
}
